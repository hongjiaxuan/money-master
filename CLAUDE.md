...
## 目前狀態（115/09/21 更新）
- **最新（v5.74，待使用者試用後才部署）**：設定頁重新分區塊＋可折疊，減少一長串扁平列表的雜亂感——
...
- **前一階段（v5.73，待使用者試用後才部署）**：信用卡對帳歸期覆蓋值影響還款金額＋已繳本期後到期提醒不再誤報＋帳戶卡片名稱過長截斷——
