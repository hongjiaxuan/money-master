...
## 目前狀態（115/07/15 更新）
- **做到哪**：新增**退款與作廢**＋**專案/事件記帳**（v5.24，分支 `claude/loan-tracking-invoice-features-8grf8p`）。
  - **退款/作廢**：TransactionCard 左滑「退款」→ RefundModal（全域，經 `openRefund` context 觸發）；產生 `#退款` transfer（虛擬帳戶 `external_refund`）還原帳戶餘額，並改寫原交易 `splitMyShare = amount − refundedAmount` 讓所有統計自動沖銷（零統計公式改動）；`#退款`/`#作廢` 加入 SYSTEM_TAGS；`#作廢` 列表灰階刪除線；刪退款 transfer 會 `restoreOriginalFromRefund` 還原。僅適用 expense。
  - **專案記帳**：新資料類型 `mm_projects` + 交易 `projectId`（比照 linkedGoalId，全類型適用）；ProjectManager（CRUD/預算/封存）+ ProjectDetailView（總支出/收入/淨額/分類佔比）；SettingsView 入口；重用 SavingsGoal 骨架。
  - Playwright 煙霧測試：專案 6 項 + 退款/作廢/刪除還原 10 項全過；借還款 regression 22 項仍全過。**注意：gh-pages 部署待 Windows 端跑 deploy.bat（或 Linux 端授權 force push）**
- **前一階段**：借還款追蹤（v5.23，`mm_debts`/DebtManager/`external_debt`/`#借貸`），已合併 PR #1 並部署上線
- **前前階段**：修正部署 — gh-pages 補齊至 v5.22（三方代墊功能 + 6 項資產一致性修正）
...
- **語言**：繁體中文介面
- **SW 版本**：`money-master-v5.24`（sw.js）
...
### sw.js 版本號規則
每次更新 `index.html` 時同步遞增，目前為 `v5.24`：
```js
const CACHE_NAME = 'money-master-v5.24';
```
