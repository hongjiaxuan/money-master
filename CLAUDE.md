...
## 目前狀態（115/07/14 更新）
- **做到哪**：新增**借還款追蹤**功能（v5.23，開發於分支 `claude/loan-tracking-invoice-features-8grf8p`）— 新資料類型 `mm_debts`（對象制流水帳：lend/borrow/collect/repay）、DebtManager + DebtEntryModal 元件、AssetsView「借貸往來」區塊、虛擬帳戶 `external_debt`、`#借貸` transfer 交易（統計零污染）；順手修 3 處既有漏洞：handleExportData 漏 `splitContacts`、handleManualRestore 的 removeItem 清單與寫回漏 `mm_split_contacts`。已用 Playwright 煙霧測試 22 項全過（借出/部分收回/結清/刪除回滾/備份含 debts）。**注意：gh-pages 部署待 Windows 端跑 deploy.bat**
- **前一階段**：修正部署 — gh-pages 補齊至 v5.22（三方代墊功能 + 6 項資產一致性修正）
...
- **語言**：繁體中文介面
- **SW 版本**：`money-master-v5.23`（sw.js）
...
### sw.js 版本號規則
每次更新 `index.html` 時同步遞增，目前為 `v5.23`：
```js
const CACHE_NAME = 'money-master-v5.23';
```
