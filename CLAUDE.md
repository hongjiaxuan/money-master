...
- **最新（v5.65，待使用者試用後才部署）**：快速記帳桌面小工具——MacroDroid 開啟 App 網址帶參數，自動跳出快速記帳選單——
  ...
  - **手機端設定（不在這個 repo 範圍）**：MacroDroid 建一個小工具（或用既有 Macro 加一個「顯示快捷方式」/Widget 觸發條件），動作選「開啟網址」，網址填 `https://hongjiaxuan.github.io/money-master/?quickadd=1`。**這裡有一個不確定因素、需要使用者自己實測**：這個網址開啟後究竟是叫起你已安裝到主畫面的 PWA 獨立視窗、還是開一個瀏覽器分頁，取決於 Android/Chrome 對這個網址範圍的 PWA 關聯設定，沒辦法在這裡保證，需要使用者實際建好小工具點一次才知道
  ...
  - Playwright 新增 `smoke43.js`（6 項斷言）：帶 `?quickadd=1` 開啟正確不用點「+」就自動彈出快速記帳選單；觸發後網址參數正確被清除；點選有預設金額的模板正確一鍵記帳、交易欄位正確、選單自動關閉；不帶參數正常開啟時選單正確不會自動彈出（回歸，避免誤觸擾民）。刻意讓新增的 `useEffect` 失效重跑測試，確認會失敗後才確認修正、還原（逐位元組比對確認還原後檔案與修正版完全一致）。既有回歸 `smoke29.js`~`smoke42.js` 全數維持全過（連同新測試共 202 項斷言全過），`node verify_build.js` JSX 編譯通過
