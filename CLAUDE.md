...
## 目前狀態（115/09/21 更新）
- **最新（v5.73，待使用者試用後才部署）**：信用卡對帳歸期覆蓋值影響還款金額＋已繳本期後到期提醒不再誤報＋帳戶卡片名稱過長截斷——
  ...
  - **本輪未做（使用者原始回報還有另外兩點，屬於獨立的設計決策而非明確 bug，待另外用 `AskUserQuestion` 確認方向後再處理，不跟這輪的三個具體 bug 修正混在一起交付）**：①「設定頁面的排序以及資訊太過雜亂，思考如何整理」——需要先確認使用者想要的分組/收合方式；②「針對所有圖示部分，是否可以改用手繪圖片上傳替換」——牽涉整套 `IconPaths` SVG 圖示系統全面替換，需要先確認素材/範圍/交付方式，屬於大範圍設計異動
...
- **語言**：繁體中文介面
- **SW 版本**：`money-master-v5.73`（sw.js）
...
```js
const CACHE_NAME = 'money-master-v5.73';
```
