...
## 目前狀態（115/09/25 更新）
- **最新（v5.76，已合併並部署上線）**：模組3：TransactionModal 編輯模式改用原地內嵌卡片選單（分類/帳戶/分帳方式），不再跳全螢幕扇形選單——使用者試用後確認「合併部署」，已 merge 進 `main` 並部署到 `gh-pages`，逐位元組比對確認部署內容與本機一致——
