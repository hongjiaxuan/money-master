...
## 目前狀態（115/09/25 更新）
- **最新（v5.75，已合併並部署上線，PR #44）**：會計雙表重建——ReportsView「財務體檢」Tab 改建成「財務中心」（資產負債表＋損益表），移除首頁「上月財務回顧」卡片與標籤統計 Tab——
...
- **前一階段（v5.74，已合併並部署上線，PR #43，與 v5.73 一併）**：設定頁重新分區塊＋可折疊，減少一長串扁平列表的雜亂感——
  ...
  - **明確不做（另一半回報的第三點，待素材/範圍確認後再處理）**：「所有圖示改用手繪圖片上傳替換」——經 `AskUserQuestion` 確認目前還沒有現成的手繪圖片素材、只是想法，全面替換整套 `IconPaths` SVG 系統是牽涉全 App 的大改動，等使用者準備好素材/確認範圍後再另外排一輪
  - **⚠️ 連帶修正既有回歸測試**：`信用卡優惠管理` 這輪被歸進「進階與財務工具」預設收合區塊，`smoke34.js`／`smoke35.js`／`smoke36.js`／`smoke38.js`／`smoke39.js`／`smoke40.js`／`smoke41.js`／`smoke44.js` 共 8 支既有測試原本從設定頁直接點擊「信用卡優惠管理」文字（假設一進設定頁就可見），這輪同步在各測試「進入設定頁」之後、「點擊信用卡優惠管理」之前補上一行展開「進階與財務工具」的動作——沿用本專案既有慣例（v5.57 手動新增區塊改可折疊時，同樣的模式已經對 `smoke34.js` 做過一次），非新手法
  - Playwright 新增 `smoke48.js`（11 項斷言）：三個新標題正確顯示；「記帳設定」預設展開、5 個項目可見；「進階與財務工具」預設收合、儲蓄目標/信用卡優惠管理不可見；點擊展開後兩者正確出現；展開後點擊「儲蓄目標」仍正確導覽進入該頁面（回歸，重組不影響既有點擊行為）；重新進入設定頁後展開狀態維持（component state 正常）。刻意將 `showAdvancedSettings` 預設值改回 `true` 重跑測試，確認恰好 4 項收合相關斷言失敗（其餘 7 項不受影響，含展開後續動作因找不到已展開內容而逾時中止，佐證測試有效攔到真正的收合邏輯）後才確認修正、還原（逐位元組比對確認還原後檔案與修正版完全一致）。既有回歸 `smoke29.js`~`smoke41.js`／`smoke44.js`~`smoke47.js` 全數維持全過（`smoke40.js` 1 項既存排隊機制時序斷言，非本次引入），`node verify_build.js` JSX 編譯通過
- **前一階段（v5.73，已合併並部署上線，PR #43，與 v5.74 一併）**：信用卡對帳歸期覆蓋值影響還款金額＋已繳本期後到期提醒不再誤報＋帳戶卡片名稱過長截斷——
  - **使用者回報（附兩張截圖，設定頁與資產頁信用卡清單）**：這輪同時回報三件事，前兩件都是「對帳/還款/到期提醒」這條鏈路裡三個各自獨立的落差，第三件是純版面問題：
    1. 「信用卡與負債的帳單歸屬，目前有設置對帳功能，若非期帳單可以移動到下一期，但在點選還款時，依舊會依造原設定日期去計算，應該要依照對帳時的確認金額」
    2. 「還款帳單月份日期確認，假設8月信用卡預計於9/24，已提前於9/10，到9/24前還是會跳出提醒尚未繳納，但實際上剩餘金額是9月信用卡的尚不需繳納」
    3. 「因為使用信用卡優化搜尋功能，信用卡的名稱會較長，導致整體方格占比太大」（截圖顯示「台新RICHART卡」帳戶名稱換行成 3 行，比其他帳戶方格高很多）
  - **根因1（對帳「移到下期」覆蓋值沒有被還款 Modal 採用）**：`ReconcileView`（v5.41 新增）的手動「移到上期／下期」只把 `reconcileCycleStart` 這個 optional 欄位寫進交易本身，供 `ReconcileView` 自己的分組判斷用；但 `openRepayModal`（決定「本期帳單」按鈕金額的計算）從頭到尾只看交易的原始 `date` 跟結帳日比較，完全沒讀過這個覆蓋值——代表使用者在對帳頁辛苦手動移期矯正銀行入帳時間差，回到還款畫面時「本期帳單」金額完全沒跟著變，等於做白工
  - **根因2（到期提醒沒有「已繳本期」的記憶）**：`getDueWarning(dueDay)` 是純日曆天數計算（`目前日期` 距 `dueDay` 還有幾天），全 App 沒有任何地方記錄「這期到底繳過沒」，只能單看「今天是幾號」判斷要不要跳提醒。使用者提前繳清本期後，若在到期日前又有新一期的刷卡讓 `balance` 變回負值，`dueDay` 那個日曆天數判斷完全沒有變、提醒還是會跳出來，即使那筆負餘額根本是下一期、還沒到繳款期限
  - **根因3（帳戶名稱過長換行撐高卡片）**：`AssetsView` 帳戶列（`renderAccountCard`）的名稱一直是原生文字，沒有任何寬度限制或截斷處理；v5.60 選卡推薦上線後使用者開始習慣填寫更長、更具描述性的實際卡名（方便自己一眼看出優惠對應），長名稱在寸土寸金的手機版面上直接換行，讓那一列比其他帳戶明顯高一截，版面不齊
  - **🔴 修正1**：`openRepayModal`（`AssetsView`）計算「未出帳金額」的 filter 條件新增覆蓋值優先判斷——`if (t.reconcileCycleStart) return t.reconcileCycleStart === lastBillDateStr; return t.date > lastBillDateStr;`，跟 `ReconcileView` 自己的歸期判斷邏輯完全一致（同一套「覆蓋值優先於原始日期」規則，不是另外發明一套）；`repayModal` state 新增 `billCycleStart` 欄位記錄本期結帳日字串，供修正2 使用
  - **🔴 修正2（新增 `Account.lastRepaidBillPeriod` 欄位）**：`confirmRepay`（一般繳費確認）與資產頁卡片上的「全額結清」快捷按鈕，只要這次繳費金額涵蓋本期已出帳金額（`本期帳單`／`全額結清`／`自訂金額 ≥ 本期已出帳` 都算），就用 `setAccounts(prev => prev.map(...))`（**刻意不用 `handleSaveAccount`**——那個 handler 會拿 `accData.balance` 跟現有 `balance` 算差額、只要不為 0 就自動建一筆 `#調整` 交易，這裡是在 `handleSaveTransaction` 已經正確改動餘額之後才追加寫入一個跟餘額無關的標記欄位，若沿用 `handleSaveAccount` 傳入任何 balance 快照值都會被誤判成「餘額被改了」而產生一筆不該存在的調整紀錄，直接用已在 `DataContext.Provider` 匯出、但沒被列進 CLAUDE.md 文件清單的 `setAccounts` 原始 setter 才是零副作用的正確寫法）把該信用卡的 `lastRepaidBillPeriod` 設為本期結帳日字串；`getDueWarning` 簽名從只收 `dueDay` 改收整個 `acc`，多一段判斷：`billDay` 有值時先算出目前週期起始日，若等於 `lastRepaidBillPeriod` 就直接回傳 `null`（不管 `balance` 現在是不是負的，都不再誤判「本期未繳」），到下一期帳單出帳（`currentCycleStart` 換成新的一天）前都不會再跳提醒；三處呼叫端（卡片徽章、urgent banner 篩選、banner 明細行）同步改傳整個帳戶物件
  - **🟡 修正3**：`renderAccountCard` 的名稱+圖示區塊補上 flex 截斷慣用手法——外層容器加 `min-w-0 flex-1`、名稱文字包一層 `<span className="truncate">`（單行截斷加省略號，到期徽章維持完整不截斷、加 `shrink-0`）、右側按鈕/餘額/編輯圖示的容器也加 `shrink-0`（不被擠壓變形），長名稱不再讓整列因換行而變高
  - **🐛 測試過程中發現並修正的測試撰寫陷阱（非 App bug，記錄避免未來新測試重蹈覆轍）**：第一版 `smoke47.js` 驗證到期提醒消失時用 `page.locator('body').textContent()` 取文字比對，結果無論修正生效與否都判斷「提醒存在」，一度誤以為修正沒生效、耗費大量除錯時間反覆檢查 `getPrevBillDate`／`lastRepaidBillPeriod` 寫入是否正確（皆已用 `page.evaluate` 直接驗證過是對的）。真正原因：`index.html` 的 `<div id="root">` 跟 `<script type="text/babel">`（整份 JSX 原始碼）是 `<body>` 底下的兩個兄弟節點，`.textContent()` 會把兩者的文字內容都串起來——只要原始碼裡剛好寫死出現過同一個字串字面值（這裡是 `warning.days === 0 ? '今日還款' : ...` 這行三元運算子），比對永遠會誤判成 true，跟畫面實際有沒有渲染出來完全無關。改用 `page.locator('#root').textContent()`（只取 React 掛載點底下的渲染內容，不含旁邊那顆巨大的原始碼 `<script>` 節點）後才正確反映真實渲染狀態。**日後任何用 `body` 整段文字比對「某段文字是否顯示」的測試，若比對字串恰好也出現在原始碼字面值裡（尤其是三元運算子/字串常數這類），都要改用 `#root` 而非 `body`，否則斷言會失去意義**
  - Playwright 新增 `smoke47.js`（10 項斷言）：三筆交易混合驗證「本期帳單」金額正確排除已用「移到下期」覆蓋歸期的那一筆（$500 而非未修正前的 $800）、「本次繳納金額」正確預帶入該值；繳費前 dueDay 在 2 天內正確顯示「剩 2 天」badge 與頂部 urgent banner；用「本期帳單」金額確認繳費後，即使 `balance` 因未出帳金額仍為負，badge 與 banner 都正確消失（佐證是「已繳本期」判斷生效、非單純看 balance）；長帳戶名稱正確套用 `truncate`、右側按鈕群組正確套用 `shrink-0`。刻意讓 `lastRepaidBillPeriod` 比對條件失效重跑測試，確認恰好 2 項到期提醒消失相關斷言失敗（其餘 8 項不受影響）後才確認修正、還原（逐位元組比對確認還原後檔案與修正版完全一致）。既有回歸 `smoke29.js`~`smoke41.js`／`smoke44.js`~`smoke46.js` 全數維持全過（`smoke40.js` 1 項既存排隊機制時序斷言，非本次引入），`node verify_build.js` JSX 編譯通過
  - **本輪未做（使用者原始回報還有另外兩點，屬於獨立的設計決策而非明確 bug，不跟這輪的三個具體 bug 修正混在一起交付）**：①「設定頁面的排序以及資訊太過雜亂，思考如何整理」——已於下一輪 v5.74 用 `AskUserQuestion` 確認方向並完成（分區塊＋可折疊）；②「針對所有圖示部分，是否可以改用手繪圖片上傳替換」——已於下一輪 v5.74 用 `AskUserQuestion` 確認，目前還沒有現成素材，暫緩
- **前一階段（v5.72，已合併並部署上線，PR #41）**：修正選卡推薦文字搜尋找不到只出現在條件說明裡的店家名稱——
...
- **前一階段（v5.69，已合併並部署上線，PR #37，與 v5.70 一併）**：四項小修——GAS 併發鎖＋Gemini Key 移出 URL＋帳戶類型擴充（過渡/虛擬）＋分帳明細圖片雙層排序＋精簡日期——使用者已將 GAS 新版本貼上 Apps Script 並部署（併發鎖與 Gemini Key header 化才會真的生效）——
  ...
  - **🟡 新增能力3（帳戶類型擴充：過渡/虛擬）**：`AccountModal` 類型選單（原 5 選項 `grid-cols-5`）新增第 6 個 `['transit','過渡/虛擬']`（改 `grid-cols-6`）；`AssetsView` 的 `groupAccounts`／`getGroupType` 補上 `transit` 分組；資產/負債/淨資產計算（`assetSummary`）完全不用改——純看 `balance` 正負號加總、型別無關，過渡帳戶的正/負餘額自動正確併入總資產/總負債/淨資產。`AssetsView` 清單渲染在既有 5 個固定分組之後新增一個可折疊區塊（新 state `showTransitAccounts`，預設收合，比照 `CardRewardManager` 的折疊慣例），標題「過渡／虛擬帳戶（N）」＋展開/收合箭頭，點擊切換，收合時不佔主畫面版面、展開才看到明細
  - **🟡 新增能力4（`AccountDetailView` 轉帳視角校正）**：`TransactionCard` 新增選填 prop `contextAccountId`（其他呼叫端如 `HomeView`／`SplitManager` 不傳，行為完全不受影響，純附加）；`transaction.type==='transfer' && contextAccountId` 時依 `contextAccountId` 是轉出方還是轉入方，動態顯示「↑ 轉出至：{目標帳戶}」紅色「-」或「↓ 轉入自：{來源帳戶}」綠色「+」，取代原本一律中性灰色、看不出方向的顯示；`AccountDetailView` 呼叫 `<TransactionCard>` 時新增 `contextAccountId={accountId}`
  - **🟡 新增能力5（分帳明細圖片雙層排序＋精簡日期）**：`buildReceiptLines` 既有的 4 個分帳類別分組（v5.42）之後新增第二層排序——各分組內部依交易日期由舊到新排序（`localeCompare`，`YYYY-MM-DD` 字典序等同時間序）；`handleExportImage` 組 `itemsToDraw` 補上 `date` 欄位；`drawReceipt` 每行畫面在項目名稱前補畫精簡日期（`MM/DD`，灰色），並依日期文字實際量測寬度動態縮減名稱截斷寬度，避免擠到彩色小標籤或金額。**保留既有代購/全額/三方/多人彩色小標籤不動**（已用 `AskUserQuestion` 確認：v5.42 就是因為完全沒標籤、代購跟多人分不出來才加上去的，這輪只加日期不刪標籤）
...
- **前一階段（v5.67，已合併並部署上線）**：記帳畫面數字鍵縮小、跟快速記帳對齊一致——
  ...
  - 用既有「真實 Tailwind 編譯＋Playwright 截圖」管線截圖確認縮小後的實際效果（含補齊本輪散失的 `tailwindcss` npm 套件），既有回歸 `smoke29.js`~`smoke42.js` 全數維持全過（純 CSS 調整、196 項斷言全過），`node verify_build.js` JSX 編譯通過
- **前一階段（v5.66，已合併並部署上線）**：移除快速記帳桌面小工具（v5.65 剛上線即取消）——
  - **使用者實機試用後判斷**：部署後使用者指出「還是要點一個開啟，然後只是單純地打開快速記帳頁面，這樣與直接點選 App 打開是一樣的道理」——小工具本身仍需要一次點擊才能啟動，啟動後也只是把「點+號」這個動作省掉，實際節省的操作幅度太小，不值得維護這條額外路徑，要求取消
  - **🔴 移除**：`MainLayout` 內判斷網址 `?quickadd=1` 參數＋監聽 `popstate`/`visibilitychange` 的整段 `useEffect`（v5.65 新增）整個刪除，`grep -c "quickadd" index.html` 確認為 0，沒有殘留。移除前先確認過**這段程式碼本身是不會被誤觸發的死碼**（只在網址帶特定參數時才啟動，使用者平常開 App 不會碰到），移除純粹是配合使用者「不留用不到的功能」的既有習慣，非修 bug
  - Playwright 移除隨 v5.65 新增、現在已無對應功能的 `smoke43.js`；既有回歸 `smoke29.js`~`smoke42.js` 全數維持全過（196 項斷言全過），`node verify_build.js` JSX 編譯通過
- **前一階段（v5.65，短暫上線後於 v5.66 移除）**：快速記帳桌面小工具——MacroDroid 開啟 App 網址帶參數，自動跳出快速記帳選單——
...
- **語言**：繁體中文介面
- **SW 版本**：`money-master-v5.75`（sw.js）
...
// Account
{
  ...
  lastRepaidBillPeriod              // liability 用（v5.73，選填）：最近一次繳費涵蓋本期已出帳金額時，
                                     // 記錄本期結帳日字串（getPrevBillDate(acc,0) 的回傳值），供 getDueWarning
                                     // 判斷「這期到底繳過沒」——純看 balance/日期算不出來，balance 隨時可能因
                                     // 下期新刷卡又變回負數；只能透過 setAccounts 原始 setter 寫入（勿用
                                     // handleSaveAccount，那個 handler 會拿 balance 差額自動建 #調整 交易）
}
...
### sw.js 版本號規則
每次更新 `index.html` 時同步遞增，目前為 `v5.75`：
```js
const CACHE_NAME = 'money-master-v5.75';
```
